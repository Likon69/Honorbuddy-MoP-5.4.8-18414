...
namespace ProfileHelper
{
    ...
    {
        public IntPtr chk = new IntPtr(0);
        public ToolForm()
        {
            InitializeComponent();
        }
    ...
    Where(o => o.BaseAddress != chk && o.IsValid).OrderBy(o => o.Distance).ToList();
            ...
            dgwNpcs.DataSource = ObjectManager.GetObjectsOfType<WoWUnit>(false, false).
                Where(u => u.BaseAddress != chk && u.IsValid).OrderBy(u => u.Distance).ToList();
