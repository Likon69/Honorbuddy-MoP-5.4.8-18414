...
namespace HighVoltz
{
    ...
    {
        private readonly int index; // index to ProfessionbuddyBot.Instance.TradeSkillList
        ...
        public TradeSkillListView(int index)
        {
            ...
            NameColumn.HeaderText = ProfessionbuddyBot.Instance.Strings["UI_Name"];
            ...
            Text = ProfessionbuddyBot.Instance.TradeSkillList[index].Name;
            // populate the controls with data
            CategoryCombo.Items.Add(""); // blank line will show all headers...

            foreach (var kv in ProfessionbuddyBot.Instance.TradeSkillList[index].KnownRecipes)
            {
        ...
        private void TradeDataView_SelectionChanged(object sender, EventArgs e)
        {
            if (MainForm.IsValid)
            {
                ...
                if (TradeDataView.SelectedRows.Count > 0)
                {
                    ...
                    Recipe _recipe = ProfessionbuddyBot.Instance.TradeSkillList[index].KnownRecipes[cell.RecipeID];
	...
	internal class TradeSkillRecipeCell
	{
        ...
        {
            get { return ProfessionbuddyBot.Instance.TradeSkillList[TradeSkillIndex].KnownRecipes[RecipeID].Name; }
        }
        ...
        {
            get { return ProfessionbuddyBot.Instance.TradeSkillList[TradeSkillIndex].KnownRecipes[RecipeID]; }
        }
